...
***example 1 (upper left)***: The equation for this response surface is a second-order polynomial in *A* and *B* without an interaction between the two factors. 
...
***example 3 (lower left)***: The equation for this response surface is a second-order polynomial in *A* and *B* with an interaction between the two factors. 
...
***example 4 (lower right)***: The equation for this response surface is a second-order polynomial in *A* and *B* without an interaction between the two factors.
...
There are a variety of functions in `R` to display data in three dimensions, such as the `persp` function used in this module. Such functions use two angles---theta and phi---to define the direction in which you view the data. These angles are typically defined only as the azimuthal angle and the colatitude angle. Unless you know how to navigate by the stars, however, you likely are not familiar with the practical meaning of these terms; thus, we describe them here as the angle by which we rotate the *xy*-plane around the *z*-axis (the azimuthal angle) and the angle by which we tilt the *z*-axis relative to the original *xy*-plane (the colatitude angle).

#### Boundary Conditions for Searching Algorithms

If left to itself, a searching algorithm may wander into factor levels that are not chemically or physically possible. For example, the spot test for vanadium described in the introduction has two factors, one for the number of drops of $\ce{H2O2}$ to use and one for the number of drops of $\ce{H2SO4}$ to use. As a negative number of drops is not physically possible, the searching algorithm must reject any set of factors that includes a factor with a negative response, and pivot to a new direction. This is what it means to set and to apply a boundary condition.  

#### Additional Resources

For an introduction to response surfaces and the mathematical modeling of response surfaces, see Chapter 14 in [Analytical Chemistry 2.1](https://chem.libretexts.org/Bookshelves/Analytical_Chemistry/Analytical_Chemistry_2.1_(Harvey)/14%3A_Developing_a_Standard_Method/14.01%3A_Optimizing_the_Experimental_Procedure) and Chapter 9 in [Chemometics Using R](https://chem.libretexts.org/Bookshelves/Analytical_Chemistry/Chemometrics_Using_R_(Harvey)/09%3A_Gathering_Data). Another useful text is Stanley. N. Deming and Stephen L. Morgan, *Experimental Design: A Chemometric Approach*, Elsevier: Amsterdam, 1987.

The description of the spot test for vanadium, included in the module's introduction, is based on the article ["Simplex Optimization of Chemical Systems," Shavers, C. L.; Parsons, M. L.; Deming, S. N. *J. Chem. Educ.* **1979**, *56*, 307--309](https://doi.org/10.1021/ed056p307).
